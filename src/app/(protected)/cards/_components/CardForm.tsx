...
export default function CardForm({
    ...
    async function onDelete(cardToDelete: TFetchedCardWithChildTransactionCount) {
        ...
        <Dialog open={dialogMode !== null} onOpenChange={(isOpen) => !isOpen && setDialogMode(null)}>
            ...
            <DialogContent className="max-w-xl bg-color-bg border-color-border">
                ...
                {dialogMode === 'Delete' && (
                    <>
                        <DialogHeader>
                            <DialogTitle>{`Deleting '${cardTobeEdited?.cardName}' Card`}</DialogTitle>
                        </DialogHeader>
                        <p>{cardTobeEdited?.childTransactionsCount} transaction(s) linked to this card will remain, but they will no longer be associated with any card.</p>
                        <p>Are You Sure?</p>
